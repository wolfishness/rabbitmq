package com.ljs.producer.sender.reply;
...
//	@Value("${java.rabbitmq.send.service.routingKey}")
	 ...
	 * 如果送达到了exchange,会回调该方法,如果消息，队列，交换机都设置了持久化，那么消息
	 * 在持久化到磁盘后，才会ack给生产者,也就是说生产者收到了ack后，消息肯定是可靠的了，已经 到磁盘了
	 */
	@PostConstruct
	public void init() {
		/**
		...
		rabbitTemplate.setConfirmCallback((correlationData, ack, cause) -> {
			if (null != messageMap && !messageMap.isEmpty()) {
				if (null != cause && !"".equals(cause)) {
					...
					rabbitTemplate.convertAndSend(SEND_EXCHANGE, ROUTING_KEY, message, correlationData);
				} else {
	...
	public void syncSend(String receiveMessage) {
		Message message = MessageBuilder.withBody(receiveMessage.getBytes()).setContentType("application/json").build();
		// 同步等待的超时时间
		rabbitTemplate.setReplyTimeout(3 * 1000);
		Object receiveObject = rabbitTemplate.convertSendAndReceive(SEND_EXCHANGE, ROUTING_KEY, message);
		System.out.println("生产者收到消费者返回的消息:" + receiveObject);
	}

	/**
	 * 异步发送消息， 异步发送，性能更高，但是无法知道消息是否发送到了exchange,可以开启生产端的重试机制
	 * spring.rabbitmq.template.retry.enabled=true，默认是false,另外
	 * 重试机制默认是重试3次，每次间隔一定时间再次重试,
	 * 
	 * @param receiveMessage
	 */
	public void asyncSend(String receiveMessage) {
		...
		Message message = MessageBuilder.withBody(receiveMessage.getBytes()).setContentType("application/json")
		...
		try {
			...
			// 最后会调用到 void basicPublish(String exchange, String routingKey, boolean
			// mandatory,
